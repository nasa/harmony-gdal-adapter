...
from os.path import basename, dirname, isdir, isfile, join as path_join, splitext
...
def create_shapefile_with_box(box, projection, shapefile):
    ...
    out_layer = out_data_source.CreateLayer(
        "boundingbox", out_spatial_ref, geom_type=ogr.wkbPolygon
    )
...
def convert_to_multipolygon(infile: str, outfile: str, buf=None):
    ...
    with fiona.open(outfile, "w", **meta) as fd_outfile:
        ...
        if polygons.geometryType() == "Polygon":
            fd_outfile.write({"geometry": mapping(polygons), "properties": {"id": 0}})
        else:
            for index_polygon, polygon in enumerate(polygons):
                fd_outfile.write(
                    {"geometry": mapping(polygon), "properties": {"id": index_polygon}}
                )
...
def shapefile_boxproj(
    shapefile: str, input_file: str, outputfile: str
