...
class TestAddToList(TestCase):
    ...
    def test_add_to_result_stacks_if_stackable_with_netcdf(
        self, is_stackable_mock, stack_multi_mock
    ):
    ...
    @patch.object(HarmonyAdapter, "stack_multi_file_with_metadata")
    @patch.object(HarmonyAdapter, "is_stackable")
    def test_add_to_result_raises_if_unstackable_with_netcdf(
        self, is_stackable_mock, stack_multi_mock
    ):
        filelist = [random_file(), random_file()]
        dstdir = random_file()
    ...
    @patch.object(HarmonyAdapter, "stack_multi_file_with_metadata")
    @patch.object(HarmonyAdapter, "is_stackable")
    def test_add_to_result_stacks_with_png(self, is_stackable_mock, stack_multi_mock):
        filelist = [random_file(), random_file()]
        dstdir = random_file()
    ...
    @patch.object(HarmonyAdapter, "stack_multi_file_with_metadata")
    @patch.object(HarmonyAdapter, "is_stackable")
    def test_add_to_result_stacks_with_jpeg(self, is_stackable_mock, stack_multi_mock):
        filelist = [random_file(), random_file()]
        dstdir = random_file()
...
class TestIsStackable(TestCase):
    ...
    def test_stackable_diff_geotransform_returns_false(self, mock_gdal_open):
        ...
        mock_geotiff_two.GetRasterBand.return_value = mock_raster_band_two
        mock_geotiff_two.GetProjection.return_value = self.projection
        mock_geotiff_two.GetGeoTransform.return_value = (-5000, 0, 500, 5000, 0, -500)
