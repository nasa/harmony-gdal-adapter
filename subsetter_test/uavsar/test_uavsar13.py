...
# South America: Spatial subset larger than granule bounding box
@pytest.fixture()
def info():
    global expected
    global product
...
def test_uavsar_status(harmony_url_config):
    ...
    if harmony_url_config.env_flag == 'prod':
        granule_id = 'G1223713114-ASF'
    else:
        granule_id = 'G1233284356-ASF'

    harmony_url = base + uavsar_id + '/ogc-api-coverages/1.0.0/collections/all/coverage/rangeset?subset=lat(-40.2:-39.0)&subset=lon(-72.4:-71.5)&format=image%2Ftiff&granuleID=' + granule_id
    global outfile
    outfile = harmony_url_config.env_flag + '_uavsar_query13.tiff'
