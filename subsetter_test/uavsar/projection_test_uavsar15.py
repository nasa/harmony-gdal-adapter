...
# Project to EPSG:2230, all variables
@pytest.fixture()
def info():
    global expected
    global product
    expected =  {'cs': 'Projected',
                 'proj_cs':'NAD83 / California zone 6',
                 'gcs': 'WGS 84',
                 'authority': 'EPSG',
                 'proj_epsg': '2230',
                 'gcs_epsg': '4326',
                 'subset': [32.75, 33.34, -117.47, -117.08],
                 'bands': 3,
...
def test_uavsar_status(harmony_url_config):
    ...
    if harmony_url_config.env_flag == 'prod':
        granule_id = 'G1368897638-ASF'
    else:
        granule_id = 'G1233284393-ASF'

    harmony_url = base + uavsar_id + '/ogc-api-coverages/1.0.0/collections/all/coverage/rangeset?format=image%2Ftiff&outputcrs=EPSG:2230&granuleID=' + granule_id
    global outfile
    outfile = harmony_url_config.env_flag + '_uavsar_query15.tiff'
