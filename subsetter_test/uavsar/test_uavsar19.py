...
#Test that gif file format works
@mark.uavsar    
@mark.status
def test_uavsar_status(harmony_url_config):
    ...
    harmony_url = base + uavsar_id + '/ogc-api-coverages/1.0.0/collections/Band1/coverage/rangeset?subset=lat(63.7:64.1)&subset=lon(-145.9:-145.7)&format=image%2Fgif&granuleID=' + granule_id
    global outfile
    outfile = harmony_url_config.env_flag + '_uavsar_query19'
...
@mark.uavsar
@mark.gif_out
def test_uavsar_gif_out(harmony_url_config):
    image_type =''
    path = './uavsar/uavsar_products/'
    global outfile
    outfile = harmony_url_config.env_flag + '_uavsar_query19'
