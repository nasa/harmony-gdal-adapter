...
# Africa: Equator crossing granule and spatial subset
@pytest.fixture()
def info():
    global expected
    global product
...
def test_uavsar_status(harmony_url_config):
    ...
    if harmony_url_config.env_flag == 'prod':
        granule_id = 'G1232482059-ASF'
    else:
        granule_id = 'G1233284343-ASF'

    harmony_url = base + uavsar_id + '/ogc-api-coverages/1.0.0/collections/all/coverage/rangeset?subset=lat(-0.034:0.09)&subset=lon(11.57:11.6)&format=image%2Ftiff&granuleID=' + granule_id
    global outfile
    outfile = harmony_url_config.env_flag + '_uavsar_query12.tiff'
