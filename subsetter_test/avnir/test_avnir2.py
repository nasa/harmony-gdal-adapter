...
# Prime meridian crossing spatial subset
@pytest.fixture()
def info():
    global expected
    global product
...
def test_avnir_status(harmony_url_config):
    ...
    if harmony_url_config.env_flag == 'prod':
        granule_id = 'G1817336128-ASF'
    else:
        granule_id = 'G1236469518-ASF'


    harmony_url = base + avnir_id + '/ogc-api-coverages/1.0.0/collections/Band3%2CBand4/coverage/rangeset?subset=lat(14.8:14.9)&subset=lon(-0.15:0.15)&format=image%2Ftiff&granuleID=' + granule_id
    global outfile
    outfile = harmony_url_config.env_flag + '_avnir_query2.tiff'
