...
def info():
    ...
    expected =  {'cs': 'Projected',
                 'proj_cs':'WGS 84 / NSIDC Sea Ice Polar Stereographic North',
                 'gcs': 'WGS 84',
                 'authority': 'EPSG',
                 'proj_epsg': '3413',
                 'gcs_epsg': '4326',
                 'subset': [62.88, 63.78, -171.65, -170.84],
                 'bands': 4,
...
def test_avnir_status(harmony_url_config):
    ...
    if harmony_url_config.env_flag == 'prod':
        granule_id = 'G1836353795-ASF'
    else:
        granule_id = 'G1234860786-ASF'


    harmony_url = base + avnir_id + '/ogc-api-coverages/1.0.0/collections/all/coverage/rangeset?outputcrs=EPSG:3413&format=image%2Ftiff&granuleID=' + granule_id
    global outfile
    outfile = harmony_url_config.env_flag + '_avnir_query11.tiff'
