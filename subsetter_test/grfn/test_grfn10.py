...
# China spatial subset area smaller than product bounding box
@pytest.fixture()
def info():
    global expected
    global product
...
def test_grfn_status(harmony_url_config):
    ...
    if harmony_url_config.env_flag == 'prod':
        granule_id = 'G1722540208-ASF'
    else:
        granule_id = 'G1234873982-ASF'

    harmony_url = base + grfn_id + '/ogc-api-coverages/1.0.0/collections/science%2Fgrids%2Fdata%2Fcoherence%2Cscience%2Fgrids%2Fdata%2FunwrappedPhase/coverage/rangeset?subset=lat(43.75:44.85)&subset=lon(86.85:88.15)&format=image%2Ftiff&granuleID=' + granule_id

    global outfile
    outfile = harmony_url_config.env_flag + '_grfn_query10.tiff'
