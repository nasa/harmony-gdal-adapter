...
# South America (negative lat and long) spatial subset area smaller than product bounding box
@pytest.fixture()
def info():
    global expected
    global product
...
def test_grfn_status(harmony_url_config):
    ...
    harmony_url = base + grfn_id + '/ogc-api-coverages/1.0.0/collections/science%2Fgrids%2Fdata%2Famplitude/coverage/rangeset?subset=lat(-37.2:-36.9)&subset=lon(-69.8:-68.8)&format=image%2Ftiff&granuleID=' + granule_id

    global outfile
    outfile = harmony_url_config.env_flag + '_grfn_query8.tiff'
