...
# South America (negative lat and long) spatial subset area larger than product bounding box
@pytest.fixture()
def info():
    global expected
    global product
    ...
    product = {'GDAL_CS': product_info.get('gdal_cs'),
               ...
               'GDAL_XY_SIZE': product_info.get('gdal_xy_size'),
...
def test_grfn_status(harmony_url_config):
    ...
    if harmony_url_config.env_flag == 'prod':
        granule_id = 'G1697222082-ASF'
    else:
        granule_id = 'G1233786871-ASF'

    harmony_url = base + grfn_id + '/ogc-api-coverages/1.0.0/collections/science%2Fgrids%2Fdata%2Fcoherence/coverage/rangeset?subset=lat(-39.0:-35.0)&subset=lon(-71.6:-67.5)&format=image%2Ftiff&granuleID=' + granule_id

    global outfile
    outfile = harmony_url_config.env_flag + '_grfn_query9.tiff'
...
@mark.grfn
@mark.xy_size
def test_grfn_xy_size(info):
        assert product['GDAL_XY_SIZE'] == expected['xy_size']
