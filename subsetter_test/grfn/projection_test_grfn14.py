...
# Southern California projected to EPSG 9802 (Lambert Conformal Conic), only the coherence variable and spatial subset.
@pytest.fixture()
def info():
    global expected
    global product
    expected = {'cs': 'Projected',
                'proj_cs':'Lambert Conformal Conic',
                'gcs': 'WGS 84',
                'authority': 'EPSG',
                'proj_epsg': '9802',
                'gcs_epsg': '4326',
                'subset': [33, 33.1, -115.5, -115.25],
                'bands': 1,
...
def test_grfn_status(harmony_url_config):
    ...
    harmony_url = base + grfn_id + 'ogc-api-coverages/1.0.0/collections/science%2Fgrids%2Fdata%2Fcoherence/coverage/rangeset?outputcrs=EPSG:9802&subset=lat(33:33.1)&subset=lon(-115.5:-115.25)&format=image%2Ftiff&granuleID=' + granule_id

    global outfile
    outfile = harmony_url_config.env_flag + '_grfn_query14.tiff'
