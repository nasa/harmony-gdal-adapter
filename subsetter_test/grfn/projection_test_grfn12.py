...
# Southern California projected to EPSG 2230, only the amplitude variable and no spatial subset.
@pytest.fixture()
def info():
    global expected
    global product
    expected = {'cs': 'Projected',
                ...
                'proj_epsg': '2230',
                'gcs_epsg': '4326',
                'subset': [32.57, 34.46, -116.84, -113.69],
                'bands': 1,
...
def test_grfn_status(harmony_url_config):
    ...
    harmony_url = base + grfn_id + '/ogc-api-coverages/1.0.0/collections/science%2Fgrids%2Fdata%2Famplitude/coverage/rangeset?outputcrs=EPSG:2230&format=image%2Ftiff&granuleID=' + granule_id

    global outfile
    outfile = harmony_url_config.env_flag + '_grfn_query12.tiff'
